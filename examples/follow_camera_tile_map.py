...
class Game(Pygame):
    def __init__(self):
        super(Game, self).__init__(600, 335, "Dungeon Game", fps_target=60)
        self.add_group("map")
    ...
    def load_data(self):
        ...
        self.add_object("map", TileMap(self, "data\\maps\\follow_camera\\follow_camera_level.tmx", follow_camera=True))
        self.add_object('player', Player((self.window_size.x / 2, self.window_size.y / 2), self))
        self.get_object("map").camera.follow_target = "player"
